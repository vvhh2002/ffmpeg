...
static av_cold void *nv_dlsym_prefixed(void *handle, const char *symbol, const char *prefix)
{
    char buf[50];
...
static av_cold int nv_omx_try_load(NVOMXContext *s, void *logctx,
                                ...
                                const char *libname2)
{
    ...
    s->ptr_Init                = nv_dlsym_prefixed(s->lib, "OMX_Init", prefix);
    s->ptr_Deinit              = nv_dlsym_prefixed(s->lib, "OMX_Deinit", prefix);
    s->ptr_ComponentNameEnum   = nv_dlsym_prefixed(s->lib, "OMX_ComponentNameEnum", prefix);
    s->ptr_GetHandle           = nv_dlsym_prefixed(s->lib, "OMX_GetHandle", prefix);
    s->ptr_FreeHandle          = nv_dlsym_prefixed(s->lib, "OMX_FreeHandle", prefix);
    s->ptr_GetComponentsOfRole = nv_dlsym_prefixed(s->lib, "OMX_GetComponentsOfRole", prefix);
    s->ptr_GetRolesOfComponent = nv_dlsym_prefixed(s->lib, "OMX_GetRolesOfComponent", prefix);
...
static av_cold NVOMXContext *nv_omx_init(void *logctx, const char *libname, const char *prefix)
{
...
// #endif
    ...
    NVOMXContext *omx_context;

    omx_context = av_mallocz(sizeof(*omx_context));
    if (!omx_context)
        return NULL;
    if (libname) {
        ret = nv_omx_try_load(omx_context, logctx, libname, prefix, NULL);
        if (ret < 0) {
            av_free(omx_context);
            return NULL;
        }
        ...
        for (nameptr = libnames; *nameptr; nameptr += 2)
            if (!(ret = nv_omx_try_load(omx_context, logctx, nameptr[0], prefix, nameptr[1])))
                break;
        if (!*nameptr) {
            av_free(omx_context);
            return NULL;
...
static av_cold void nv_omx_deinit(NVOMXContext *omx_context)
{
    if (!omx_context)
        return;
...
typedef struct NVOMXCodecContext {
    const AVClass *class;
    char *libname;
    char *libprefix;
    NVOMXContext *omx_context;
...
//from gstomxvideoenc.h
  int control_rate; //guint32 control_rate;
  //guint32 bitrate;
  int peak_bitrate;//guint32 peak_bitrate;
  int quant_i_frames;//guint32 quant_i_frames;
  int quant_p_frames;//guint32 quant_p_frames;
  int quant_b_frames;//;guint32 quant_b_frames;
  int iframeinterval;//guint32 iframeinterval;
  int quality_level;//guint32 quality_level;
  int SliceIntraRefreshEnable;//gboolean SliceIntraRefreshEnable;
  int bit_packetization;//gboolean bit_packetization;
  int SliceIntraRefreshInterval;//guint SliceIntraRefreshInterval;
  int vbv_size_factor;//gboolean vbv_size_factor;
  int temporal_tradeoff;//guint32 temporal_tradeoff;
  int max_frame_dist;//guint32 max_frame_dist;
  int EnableMVBufferMeta;//gboolean EnableMVBufferMeta;
  int MinQpI;//guint32 MinQpI;
  int MaxQpI;//guint32 MaxQpI;
  int MinQpP;//guint32 MinQpP;
  int MaxQpP;//guint32 MaxQpP;
  int MinQpB;//guint32 MinQpB;
  int MaxQpB;//guint32 MaxQpB;
  int set_qpRange;//gboolean set_qpRange;
//   int measure_latency;gboolean measure_latency;
 // guint64 framecount;
  //FILE *tracing_file_enc;
  //GQueue *got_frame_pt;
  int EnableTwopassCBR;//gboolean EnableTwopassCBR;
  int hw_preset_level;//guint32 hw_preset_level;
  int EnableStringentBitrate;//gboolean EnableStringentBitrate;


// from gstomxh246enc.h
    int nBFrames; //gboolean  0 1
    int insert_sps_pps;//gboolean  0 1
    int slice_header_spacing;
    int insert_aud; //gboolean  0 1
    int insert_vui; // gboolean 
    int stream_format; // h264_sf 0: H264_AVC 1:H264_BTS


} NVOMXCodecContext;

static void nv_append_buffer(pthread_mutex_t *mutex, pthread_cond_t *cond,
...
static OMX_BUFFERHEADERTYPE *nv_get_buffer(pthread_mutex_t *mutex, pthread_cond_t *cond,
                                        int* array_size, OMX_BUFFERHEADERTYPE **array,
                                        int wait)
{
    OMX_BUFFERHEADERTYPE *buffer;
...
static OMX_ERRORTYPE nv_event_handler(OMX_HANDLETYPE component, OMX_PTR app_data, OMX_EVENTTYPE event,
                                   OMX_U32 data1, OMX_U32 data2, OMX_PTR event_data)
{
    NVOMXCodecContext *s = app_data;
...
static OMX_ERRORTYPE nv_empty_buffer_done(OMX_HANDLETYPE component, OMX_PTR app_data,
                                       OMX_BUFFERHEADERTYPE *buffer)
{
    NVOMXCodecContext *s = app_data;
    if (s->input_zerocopy) {
    ...
    nv_append_buffer(&s->input_mutex, &s->input_cond,
                  &s->num_free_in_buffers, s->free_in_buffers, buffer);
    return OMX_ErrorNone;
}

static OMX_ERRORTYPE nv_fill_buffer_done(OMX_HANDLETYPE component, OMX_PTR app_data,
                                      OMX_BUFFERHEADERTYPE *buffer)
{
    NVOMXCodecContext *s = app_data;
    nv_append_buffer(&s->output_mutex, &s->output_cond,
...
static const OMX_CALLBACKTYPE callbacks = {
    nv_event_handler,
    nv_empty_buffer_done,
    nv_fill_buffer_done
};

static av_cold int nv_find_component(NVOMXContext *omx_context, void *logctx,
...
static av_cold int nv_wait_for_state(NVOMXCodecContext *s, OMX_STATETYPE state)
{
    int ret = 0;
    pthread_mutex_lock(&s->state_mutex);
...
static av_cold int nv_omx_component_init(AVCodecContext *avctx, const char *role)
{
    NVOMXCodecContext *s = avctx->priv_data;
    ...
    else if (avctx->codec->id == AV_CODEC_ID_MPEG2VIDEO)
        out_port_params.format.video.eCompressionFormat = OMX_VIDEO_CodingMPEG2;
    else if (avctx->codec->id == AV_CODEC_ID_HEVC)
        out_port_params.format.video.eCompressionFormat = NVX_VIDEO_CodingHEVC;
...
//Tegra
    ...
    char *location = strdup (ENCODER_CONF_LOCATION);
    err =OMX_GetExtensionIndex(s->handle,(char *)NVX_INDEX_PARAM_TEMPFILEPATH,&eIndex);
    CHECK(err);
    INIT_STRUCT(enc_conf_loc);
    enc_conf_loc.pTempPath = location;
    err =OMX_SetParameter(s->handle,eIndex,&enc_conf_loc);
    if (err != OMX_ErrorNone)
        av_log(avctx, AV_LOG_WARNING, "Unable to set tegra configurations\n");
    ...
    vid_param_bitrate.eControlRate   = s->control_rate;
    ...
    // 
    if (s->quant_i_frames != 0xffffffff ||
        s->quant_p_frames != 0xffffffff ||
        s->quant_b_frames != 0xffffffff) {
        
            OMX_VIDEO_PARAM_QUANTIZATIONTYPE quant_param;
            INIT_STRUCT(quant_param);
            err = OMX_GetParameter(s->handle,OMX_IndexParamVideoQuantization,&quant_param);
            if (err == OMX_ErrorNone) {
                    if (s->quant_i_frames != 0xffffffff)
                    quant_param.nQpI = s->quant_i_frames;
                    if (s->quant_p_frames != 0xffffffff)
                    quant_param.nQpP = s->quant_p_frames;
                    if (s->quant_b_frames != 0xffffffff)
                    quant_param.nQpB = s->quant_b_frames;

                    err = OMX_SetParameter(s->handle, OMX_IndexParamVideoQuantization, &quant_param);
                     if (err == OMX_ErrorUnsupportedIndex) {
                          av_log(avctx, AV_LOG_WARNING, "Setting quantization parameters not supported by the component\n");
                     }else if (err == OMX_ErrorUnsupportedSetting) {
                         av_log(avctx, AV_LOG_WARNING, "Setting quantization parameters value not supported by the component\n");
    ...
    INIT_STRUCT(oEncodeProp);
    oEncodeProp.nPortIndex =s->out_port;
    err=OMX_GetExtensionIndex(s->handle,(char *)NVX_INDEX_PARAM_VIDEO_ENCODE_PROPERTY,&eIndex);
     if (err == OMX_ErrorNone) {
      oEncodeProp.bSliceIntraRefreshEnable = s->SliceIntraRefreshEnable;
      oEncodeProp.SliceIntraRefreshInterval = s->SliceIntraRefreshInterval;
      oEncodeProp.bBitBasedPacketization = s->bit_packetization;
      oEncodeProp.bEnableMVBufferDump = s->EnableMVBufferMeta;
      oEncodeProp.bEnableTwopassCBR = s->EnableTwopassCBR;
      //FIXME:HERE
    //  if(oEncodeProp.bEnableMVBufferDump)
    //     {
    //         s->out_port->extra_data_size += sizeof (OMX_OTHER_EXTRADATATYPE)
    //             + sizeof (NVX_VIDEO_ENC_OUTPUT_EXTRA_DATA);
    //     }
         oEncodeProp.nVirtualBufferSize =
          (s->vbv_size_factor * avctx->bit_rate) / (avctx->framerate.num /
          avctx->framerate.den);

          if (s->control_rate == OMX_Video_ControlRateVariable) {
            if (s->peak_bitrate == (0))
                oEncodeProp.nPeakBitrate = 1.2f * avctx->bit_rate;
            else if (s->peak_bitrate >= avctx->bit_rate)
                oEncodeProp.nPeakBitrate = s->peak_bitrate;
          }
        err = OMX_SetParameter(s->handle,eIndex,&oEncodeProp);
        CHECK(err);
     }

     NVX_CONFIG_VIDEO_HWPRESET_LEVEL oHwPresetLevel;
     INIT_STRUCT(oHwPresetLevel);
     err=OMX_GetExtensionIndex(s->handle,(char *)NVX_INDEX_CONFIG_VIDEO_ENCHWPRESETLEVEL,&eIndex);
     CHECK(err);
     oHwPresetLevel.hwPreset =s->hw_preset_level;
     err = OMX_SetParameter(s->handle,eIndex,&oHwPresetLevel);
     CHECK(err);
//continue
    ...
    if (nv_wait_for_state(s, OMX_StateIdle) < 0) {
        av_log(avctx, AV_LOG_ERROR, "Didn't get OMX_StateIdle\n");
        return AVERROR_UNKNOWN;
    }
    err = OMX_SendCommand(s->handle, OMX_CommandStateSet, OMX_StateExecuting, NULL);
    CHECK(err);
    if (nv_wait_for_state(s, OMX_StateExecuting) < 0) {
...
static av_cold void nv_cleanup(NVOMXCodecContext *s)
{
    int i, executing;
    ...
    if (executing) {
        ...
        nv_wait_for_state(s, OMX_StateIdle);
        OMX_SendCommand(s->handle, OMX_CommandStateSet, OMX_StateLoaded, NULL);
        for (i = 0; i < s->num_in_buffers; i++) {
            OMX_BUFFERHEADERTYPE *buffer = nv_get_buffer(&s->input_mutex, &s->input_cond,
        ...
        for (i = 0; i < s->num_out_buffers; i++) {
            OMX_BUFFERHEADERTYPE *buffer = nv_get_buffer(&s->output_mutex, &s->output_cond,
                                                      &s->num_done_out_buffers, s->done_out_buffers, 1);
            OMX_FreeBuffer(s->handle, s->out_port, buffer);
        }
        nv_wait_for_state(s, OMX_StateLoaded);
    }
    if (s->handle) {
...
static av_cold int nv_omx_encode_init(AVCodecContext *avctx)
{
    NVOMXCodecContext *s = avctx->priv_data;
    int ret = AVERROR_ENCODER_NOT_FOUND;
    const char *role;
    ...
    s->omx_context = nv_omx_init(avctx, s->libname, s->libprefix);
    if (!s->omx_context)
        return AVERROR_ENCODER_NOT_FOUND;
    ...
    if ((ret = nv_find_component(s->omx_context, avctx, role, s->component_name, sizeof(s->component_name))) < 0)
        goto fail;

    av_log(avctx, AV_LOG_INFO, "Using %s\n", s->component_name);

    if ((ret = nv_omx_component_init(avctx, role)) < 0)
        goto fail;

    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        while (1) {
            buffer = nv_get_buffer(&s->output_mutex, &s->output_cond,
                                &s->num_done_out_buffers, s->done_out_buffers, 1);
...
static int nv_omx_encode_frame(AVCodecContext *avctx, AVPacket *pkt,
                            const AVFrame *frame, int *got_packet)
{
    ...
    if (frame) {
        ...
        int linesize[4];
        int need_copy;
        buffer = nv_get_buffer(&s->input_mutex, &s->input_cond,
                            &s->num_free_in_buffers, s->free_in_buffers, 1);
        ...
        if (s->input_zerocopy) {
            ...
            if (frame->linesize[0] == src_linesize[0] &&
                ...
                if (!local) {
                    ...
                    nv_append_buffer(&s->input_mutex, &s->input_cond, &s->num_free_in_buffers, s->free_in_buffers, buffer);
                    return AVERROR(ENOMEM);
                } else {
    ...
    while (!*got_packet && ret == 0 && !s->got_eos) {
        ...
        err = OMX_FillThisBuffer(s->handle, buffer);
        if (err != OMX_ErrorNone) {
            nv_append_buffer(&s->output_mutex, &s->output_cond, &s->num_done_out_buffers, s->done_out_buffers, buffer);
            av_log(avctx, AV_LOG_ERROR, "OMX_FillThisBuffer failed: %x\n", err);
            ret = AVERROR_UNKNOWN;
        }
    }
...
static av_cold int nv_omx_encode_end(AVCodecContext *avctx)
{
    NVOMXCodecContext *s = avctx->priv_data;
...
#define OFFSET(x) offsetof(NVOMXCodecContext, x)
...
static const enum AVPixelFormat nv_omx_encoder_pix_fmts[] = {
    AV_PIX_FMT_YUV420P, AV_PIX_FMT_NONE
};

static const AVClass nv_omx_mpeg4enc_class = {
    .class_name = "mpeg4_nv_omx",
    .item_name  = av_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVCodec ff_mpeg4_nv_omx_encoder = {
    .name             = "mpeg4_nv_omx",
    .long_name        = NULL_IF_CONFIG_SMALL("OpenMAX IL MPEG-4 video encoder"),
    .type             = AVMEDIA_TYPE_VIDEO,
    .id               = AV_CODEC_ID_MPEG4,
    .priv_data_size   = sizeof(NVOMXCodecContext),
    .init             = nv_omx_encode_init,
    .encode2          = nv_omx_encode_frame,
    .close            = nv_omx_encode_end,
    .pix_fmts         = nv_omx_encoder_pix_fmts,
    .capabilities     = AV_CODEC_CAP_DELAY,
    .caps_internal    = FF_CODEC_CAP_INIT_THREADSAFE | FF_CODEC_CAP_INIT_CLEANUP,
    .priv_class       = &nv_omx_mpeg4enc_class,
};

static const AVClass nv_omx_h264enc_class = {
    .class_name = "h264_nv_omx",
    .item_name  = av_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVCodec ff_h264_nv_omx_encoder = {
    .name             = "h264_nv_omx",
    ...
    .capabilities     = AV_CODEC_CAP_DELAY,
    .caps_internal    = FF_CODEC_CAP_INIT_THREADSAFE | FF_CODEC_CAP_INIT_CLEANUP,
    .priv_class       = &nv_omx_h264enc_class,
};
